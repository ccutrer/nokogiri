    ...
    module XMLSec
      class TestSigning < Nokogiri::TestCase
        ...
        def test_signing_with_rsa_key
          ...
          assert @xml.verify_signature(key: @public_key)
          assert @xml.verify_signature({ "test" => @public_key })
        end

        def test_signing_with_rsa_key_and_x509_certificate
          @xml.sign(key: @certificate_private_key,
          ...
          assert @xml.verify_signature(certificate: @certificate)
          assert @xml.verify_signature(certificates: [@certificate])
        end
        ...
        %w[rsa-sha1 rsa-sha224 rsa-sha256 rsa-sha384 rsa-sha512].each do |signature_algorithm|
          define_method(:"test_signing_with_#{signature_algorithm.tr("-", "_")}") do
            assert @xml.sign(key: @certificate_private_key,
              certificate: @certificate,
